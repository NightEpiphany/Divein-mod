...
public class DiveinClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        PlayerAnimationFactory.ANIMATION_DATA_FACTORY.registerFactory(Identifier.fromNamespaceAndPath(Divein.MOD_ID, "dive"), 1600,
                player -> new PlayerAnimationController(player,
                        (controller, state, animSetter) -> PlayState.STOP
                )
        );

        PlayerAnimationFactory.ANIMATION_DATA_FACTORY.registerFactory(Identifier.fromNamespaceAndPath(Divein.MOD_ID, "lava_dive"), 1600,
                player -> new PlayerAnimationController(player,
                        (controller, state, animSetter) -> PlayState.STOP
                )
        );
